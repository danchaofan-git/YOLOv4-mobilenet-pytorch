...
def show_pic(img, bboxes=None):
    ...
    cv2.moveWindow('pic', 0, 0)     # 移动窗口
    cv2.resizeWindow('pic', 1200,800)  # 可视化的图片大小
    cv2.imshow('pic', img)
    cv2.waitKey(500)              # 窗口显示的时间
...
if __name__ == '__main__':
    ...
    source_pic_root_path = 'VOCdevkit/VOC2007/JPEGImages-copy/'
    source_xml_root_path = './VOCdevkit/VOC2007/Annotations-copy/'
    # 这段用来删除增强后的标签
    for parent, _, files in os.walk(source_xml_root_path):
        for file in files:
            if file.find('auged') != -1:
                os.remove(source_xml_root_path+file)
    # 这段用来删除增强后的图片
    for parent, _, files in os.walk(source_pic_root_path):
        for file in files:
            if file.find('auged') != -1:
                os.remove(source_pic_root_path+file)
        ...
        for file in files:
            ...
            for each in coords:
                if each[4] == 'after':
                    ...
                    print('检测到的目标与类别文件不对应！')

            if first == 2:
                need_aug_num = 4
            elif first >=3 and after < first:
                need_aug_num = 6
            elif first >=3 and after >= first:
                need_aug_num = 5
            elif first == 1:
            ...
            while cnt < need_aug_num:
                # 增强后的图片名
                auged_pic_path = file[:-4]+ 'auged' +str(cnt)+ '.jpg'
                # show_pic(img, coords1)    # 原图
                auged_img, auged_bboxes = dataAug.dataAugment(img, coords)
                # 使用dataAugment后输出的是增强后的图片和边框信息
                cv2.imwrite(os.path.join(source_pic_root_path,auged_pic_path), auged_img)
                generate_xml(auged_pic_path, auged_bboxes, shape, source_xml_root_path)
                # generate_xml写入新的标签文件，名字和图片名一致，只是后缀不一样
                cnt += 1
