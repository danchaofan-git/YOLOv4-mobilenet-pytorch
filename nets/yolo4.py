...
class YoloBody(nn.Module):
    def __init__(self, num_anchors, num_classes, backbone="mobilenetv3", pretrained=False):
        ...
        if backbone == "mobilenetv1":
            ...
            in_filters = [40, 112, 160] # width_mult = 1时
            # in_filters = [32, 88, 120]  # width_mult = 0.75时
            # in_filters = [24, 56, 80]   # width_mult = 0.5时
    ...
    def forward(self, x):
        ...
        # P5上采样后，与P4堆叠，再进行五次卷积

        P4_upsample = self.upsample2(P4)
        P3 = self.conv_for_P3(x2)
        P3 = torch.cat([P3,P4_upsample],axis=1)
        P3 = self.make_five_conv2(P3)
        # P4上采样后，与P3堆叠，再进行五次卷积

        P3_downsample = self.down_sample1(P3)
        P4 = torch.cat([P3_downsample,P4],axis=1)
        P4 = self.make_five_conv3(P4)
        # P3下采样后，与P2堆叠，再进行五次卷积

        P4_downsample = self.down_sample2(P4)
        P5 = torch.cat([P4_downsample,P5],axis=1)
        P5 = self.make_five_conv4(P5)
        # P4下采样后，与P5堆叠，再进行五次卷积
