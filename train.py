...
def fit_one_epoch(net,yolo_losses,epoch,epoch_size,epoch_size_val,gen,genval,Epoch,cuda):
    ...
    writer = SummaryWriter('runs/'+str(backbone)+"_experiment")
    ...
    with tqdm(total=epoch_size_val, desc=f'Epoch {epoch + 1}/{Epoch}',postfix=dict,mininterval=0.3) as pbar:
        for iteration, batch in enumerate(genval):
            ...
            # writer.add_scalar('Val_loss', loss, (epoch * epoch_size + iteration))
    writer = SummaryWriter('runs/'+str(backbone)+"_experiment")
    writer.add_scalar('Val_loss', val_loss/(epoch_size+1), epoch)
...
if __name__ == "__main__":
    ...
    # 使用预训练权重可以加快模型训练的效率，预训练权重文件保存在model_data下
